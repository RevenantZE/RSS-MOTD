...
| `scripts/sync-discord-human-skins.py` | Discord 인간 스킨 스레드에서 이름과 이미지를 동기화 |
...
| `.github/workflows/sync-discord-human-skins.yml` | 인간 스킨 스레드를 15분마다 확인하고 변경 시 Pages 배포 |
...
#### Discord 인간 스킨 자동 동기화

인간 스킨 스레드 `1517461614356594700`은 다음 규칙으로 읽습니다.
...
기존 `DISCORD_BOT_TOKEN` Secret을 함께 사용합니다. 다음 Repository Variable은 생략하면 현재 인간 스킨 서버와 스레드 ID를 기본값으로 사용합니다.
...
수동 실행은 GitHub의 **Actions → Sync Discord human skins → Run workflow**에서 할 수 있습니다. 첫 실행에서는 기존 이미지 파일을 다시 변환하지 않고 Discord 첨부 파일 ID를 `data/skins/discord-human-state.json`에 연결합니다. 이후 이름이나 첨부 이미지가 바뀌면 해당 스킨만 갱신하며, 실제 변경이 있을 때만 커밋합니다.
